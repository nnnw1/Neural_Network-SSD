...
def default_box_generator(layers, large_scale, small_scale):
    ...
    for layer_idx in range(len(layers)):
        lsize = large_scale[layer_idx]
        ssize = small_scale[layer_idx]
        box1 = [ssize, ssize]
        box2 = [lsize, lsize]
        box3 = [lsize * np.sqrt(2), lsize / np.sqrt(2)]
        box4 = [lsize / np.sqrt(2), lsize * np.sqrt(2)]

        grid_size = 1 / layers[layer_idx]

        for x in range(layers[layer_idx]):
            for y in range(layers[layer_idx]):
                x_center = grid_size * x + grid_size / 2
                y_center = grid_size * y + grid_size / 2

                x_min = x_center - box1[0] / 2
                x_max = x_center + box1[0] / 2
                y_min = y_center - box1[1] / 2
                y_max = y_center + box1[1] / 2
                boxes[box_num, 0, :] = [x_center, y_center, box1[0], box1[1], x_min, y_min, x_max, y_max]

                x_min = x_center - box2[0] / 2
                x_max = x_center + box2[0] / 2
                y_min = y_center - box2[1] / 2
                y_max = y_center + box2[1] / 2
                boxes[box_num, 1, :] = [x_center, y_center, box2[0], box2[1], x_min, y_min, x_max, y_max]

                x_min = x_center - box3[0] / 2
                x_max = x_center + box3[0] / 2
                y_min = y_center - box3[1] / 2
                y_max = y_center + box3[1] / 2
                boxes[box_num, 2, :] = [x_center, y_center, box3[0], box3[1], x_min, y_min, x_max, y_max]
                
                x_min = x_center - box4[0] / 2
                x_max = x_center + box4[0] / 2
                y_min = y_center - box4[1] / 2
                y_max = y_center + box4[1] / 2
                boxes[box_num, 3, :] = [x_center, y_center, box4[0], box4[1], x_min, y_min, x_max, y_max]
...
class COCO(torch.utils.data.Dataset):
    def __init__(self, imgdir, anndir, class_num, boxs_default, train = True, image_size=320, val = False):
        ...
        split = int(len(self.img_names) * train_test_split)
        start_img = 0
        end_img = len(self.img_names)
        if self.train and self.val == False:
            start_img = 0
            end_img = split
        elif self.train == False and self.val:
            start_img = split
            end_img = len(self.img_names)
        self.img_names = self.img_names[start_img:end_img]
    ...
    def randomCrop(self, image):
        ...
        crop_pixel_w = round(crop_pixel_h * width / height)
        rand_h = np.random.uniform(0, 1)
        rand_w = np.random.uniform(0, 1)
        # start_h = np.floor(rand_h * crop_pixel_h).astype(int)
        # start_w = np.floor(rand_w * crop_pixel_w).astype(int)
        start_h = int(0)
        start_w = int(0)
        end_h = np.floor(height - rand_h * crop_pixel_h).astype(int)
        end_w = np.floor(width - rand_w * crop_pixel_w).astype(int)
        cropImg = image[start_h:end_h, start_w:end_w]
        ...
        cropPts[2] = end_h
        cropPts[3] = end_w
        # print(image.shape, cropImg.shape, cropPts)

        return cropImg, cropPts
    ...
    def __getitem__(self, index):
        ...
        output_name = self.img_names[index][:-4]
        #TODO:
        ...
        if self.train == False and self.val == False:
            img = cv2.resize(img, (self.image_size, self.image_size))
            img = img.transpose(2, 0, 1)

        # img = cv2.resize(img, (self.image_size, self.image_size))
        # img = img.transpose(2, 0, 1)
        class_stat = np.zeros([3], np.int8)
        if self.train or self.val:
            #random cropping
            if self.augmented and self.train:
                img, cropPts = self.randomCrop(img)
                crop_height, crop_width, crop_channels = img.shape
            img = cv2.resize(img, (self.image_size, self.image_size))
            img = img.transpose(2, 0, 1)

            # class_stat = np.zeros([3], np.int8)
            file = open(ann_name, "r")
            ...
            for line in lines:
                ...
                w = (float(w))
                h = (float(h))
                class_stat[class_id] += 1
                
                if self.augmented and self.train:
                    x_min = (x - cropPts[1]) / crop_width
                    y_min = (y - cropPts[0]) / crop_height
                    x_max = (x + w - cropPts[1]) / crop_width
                    y_max = (y + h - cropPts[0]) / crop_height
                else:
                    #Normalizing ground truth bounding box
                    x_min = (x / width)
                    y_min = (y / height)
                    x_max = ((x + w) / width)
                    y_max = ((y + h) / height)
        ...
        return img, ann_box, ann_confidence, height, width, output_name, class_stat
